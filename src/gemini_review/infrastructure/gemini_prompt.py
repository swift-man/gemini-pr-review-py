...
같은 원칙이 다른 영역에도 적용됩니다 — **출처를 직접 확인할 수 없는 주장은 하지 말 것**:
- 존재하지 않는 함수/메서드 호출이 있다고 추측 → 파일 안에서 그 호출을 직접 확인 못 했으면 생략
- 변수 타입을 추론할 수 없으면 타입 관련 지적 생략
- import 안 된 모듈이 쓰였다고 주장 → import 섹션에서 실제로 확인한 경우만
"""
